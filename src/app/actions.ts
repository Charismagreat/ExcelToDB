  ...
  for (const sheet of sheetResults) {
    for (const table of sheet.tables) {
      ...
      try {
        ...
        console.log(`Physically created table: ${physicalTableName}`);
      } catch (err: any) {
...
export async function addRowAction(reportId: string, rowData: any) {
  ...
  let rowCount = 0;
  // Use lastSerial if it is tracked, else fallback to db count
  if (report.lastSerial !== undefined && report.lastSerial !== null) {
      rowCount = Number(report.lastSerial);
  } else {
      try {
          const rowCountResult = await aggregateTable('report_row', 'id', 'COUNT', { filters: { reportId: String(reportId) } });
          let extracted = 0;
          if (Array.isArray(rowCountResult)) {
              if (rowCountResult.length > 0) {
                  const first = rowCountResult[0];
                  extracted = Number(typeof first === 'object' && first !== null ? (first.value !== undefined ? first.value : Object.values(first)[0]) : first);
              }
          } else if (rowCountResult && typeof rowCountResult === 'object') {
              extracted = Number('value' in rowCountResult ? (rowCountResult as any).value : Object.values(rowCountResult)[0]);
          } else {
              extracted = Number(rowCountResult);
          }
          if (!isNaN(extracted)) rowCount = extracted;
      } catch (e) {
          console.error('Failed to aggregate row count:', e);
      }
      
      // 최후의 안정망: 개수를 찾지 못했거나 초기 상태일 경우 (100건 제한 없이 전체 스캔)
      if (rowCount === 0) {
...
export async function updateReportSchemaAction(reportId: string, columns: any[], convertExistingData: boolean = false) {
    ...
    // 2. 메타데이터 업데이트
    await updateRows('report', { columns: JSON.stringify(columns) }, { filters: { id: String(reportId) } });
...
export async function addRowsAction(reportId: string, rows: any[]) {
    ...
    try {
        ...
        if (Array.isArray(rowCountResult)) {
            if (rowCountResult.length > 0) {
                const first = rowCountResult[0];
                extracted = Number(typeof first === 'object' && first !== null ? Object.values(first)[0] : first);
            }
        } else if (rowCountResult && typeof rowCountResult === 'object') {
            extracted = Number(Object.values(rowCountResult)[0]);
        } else {
            extracted = Number(rowCountResult);
        }
        if (!isNaN(extracted)) lastSerial = extracted;
    } catch (e) {
        console.error('Failed to aggregate row count for bulk bulk:', e);
    }
