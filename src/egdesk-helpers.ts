...
export async function callUserDataTool(
  ...
  if (isServer) {
    ...
    // handle potential basePath for tunneling
    const basePath = (typeof process !== 'undefined' && process.env?.NEXT_PUBLIC_EGDESK_BASE_PATH) || '';
    const proxyUrl = `/__user_data_proxy`.startsWith('/') && !`/__user_data_proxy`.startsWith('//')
  ...
  // Parse MCP response format safely
  ...
  try {
     ...
     console.error('Failed to parse EGDesk tool response as JSON:', content);
...
export async function queryTable(
  ...
  // Beautiful: extract rows so it's ready for high-level logic (e.g. results[0])
  return result?.rows || [];
}
...
export async function getTableSchema(tableName: string) {
  ...
  // Robust: unify field access if it differs between different versions of tool
  return result?.schema || result?.columns || [];
}
