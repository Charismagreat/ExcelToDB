...
export class ChartService {
     ...
     */
    static resolveDynamicValue(val: any): any {
     ...
     */
    static resolveDynamicArgs(args: any): any {
        if (!args || typeof args !== 'object') return args;
        const resolvedArgs: any = Array.isArray(args) ? [] : {};
        for (const [key, value] of Object.entries(args)) {
            if (value && typeof value === 'object') {
                resolvedArgs[key] = this.resolveDynamicArgs(value);
            } else {
                resolvedArgs[key] = this.resolveDynamicValue(value);
            }
        }
        return resolvedArgs;
    }
     ...
     */
    static resolveDynamicDescription(desc: string, args: any): string {
        if (!desc) return desc;
        let resolvedDesc = desc;
        resolvedDesc = resolvedDesc.replace(/\$TODAY(?:-(\d+))?/g, (match) => this.resolveDynamicValue(match));
        
        if (args?.startDate && args?.endDate) {
            const sDate = this.resolveDynamicValue(args.startDate);
            const eDate = this.resolveDynamicValue(args.endDate);
     ...
     */
    static mapRefreshedData(rawData: any, mapping: any): any[] {
        let newData: any[] = [];
     ...
     */
    static async loadAllPinnedCharts(): Promise<ChartConfig[]> {
        try {
     ...
     * 차트 목록을 파일에 저장합니다.
     */
    static async saveAllPinnedCharts(charts: ChartConfig[]): Promise<void> {
     ...
     * 특정 사용자의 차트를 새로고침합니다.
     */
    static async refreshUserCharts(userId: string): Promise<{ charts: ChartConfig[], hasChanges: boolean }> {
        const allCharts = await this.loadAllPinnedCharts();
        ...
        const refreshedCharts = await Promise.all(userCharts.map(async (item) => {
            if (item.config.refreshMetadata) {
                const refreshedItem = await this.refreshSingleChart(item);
        ...
        if (hasChanges) {
            ...
            await this.saveAllPinnedCharts(updatedAll);
        }
     ...
     */
    static async refreshSingleChart(item: ChartConfig): Promise<ChartConfig> {
        if (!item.config.refreshMetadata) return item;

        try {
            const { tool, args: originalArgs, mapping } = item.config.refreshMetadata;
            const args = this.resolveDynamicArgs(originalArgs);
            
            if (item.config.sourceDescription) {
                item.config.sourceDescription = this.resolveDynamicDescription(item.config.sourceDescription, originalArgs);
            }
            
            const rawData = await runAITool(tool, args);
            const newData = this.mapRefreshedData(rawData, mapping);
